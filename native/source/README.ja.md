...
このディレクトリは `irodori.firebird` のマイグレーションステージングエリアです。アクティブなネイティブエントリポイントは `src/lib.rs` にあり、共有ABIヘルパーは `src/abi.rs` にあり、エンジンの動作は `src/stub.rs` または `src/driver.rs` にあります。エンジンスペシフィックな接続/クエリ/メタデータコードは、これらのスナップショットからその動作モジュールに移動すべきです。コネクタのランタイム契約がデスクトップアプリに接続されるときに。
